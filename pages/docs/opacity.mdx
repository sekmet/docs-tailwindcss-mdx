...
<div className="flex -mx-2">
  @foreach ($page->config['theme']['opacity']->reverse() as $name => $value)
    <div className="text-sm flex-1 text-gray-700 text-center bg-gray-400 px-4 py-2 mx-2 opacity- $name ">.opacity- $name </div>
  @endforeach
</div>
@slot('code')
@foreach ($page->config['theme']['opacity']->reverse() as $name => $value)
<div className="opacity- $name ">.opacity- $name </div>
@endforeach
@endslot
@endcomponent
...
<div className="text-center">
  <div className="px-4 py-2 bg-gray-400 opacity-100 w-24 h-24 rounded-full inline-block"></div>
</div>
@endslot
@slot('sm')
<div className="text-center">
  <div className="px-4 py-2 bg-gray-400 opacity-75 w-24 h-24 rounded-full inline-block"></div>
</div>
@endslot
@slot('md')
<div className="text-center">
  <div className="px-4 py-2 bg-gray-400 opacity-50 w-24 h-24 rounded-full inline-block"></div>
</div>
@endslot
@slot('lg')
<div className="text-center">
  <div className="px-4 py-2 bg-gray-400 opacity-25 w-24 h-24 rounded-full inline-block"></div>
</div>
@endslot
@slot('xl')
<div className="text-center">
  <div className="px-4 py-2 bg-gray-400 opacity-0 w-24 h-24 rounded-full inline-block"></div>
</div>
@endslot
@slot('code')
<div className="none:opacity-100 sm:opacity-75 md:opacity-50 lg:opacity-25 xl:opacity-0 ...">
  {/* ... */}
</div>
@endslot
@endcomponent
