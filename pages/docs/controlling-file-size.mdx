...
<h2 style={{visibility: "hidden", fontSize: "0", margin: "0"}}>Overview</h2>
...
<pre className="language-html mt-4"><code><div className="text-@{{ error ? 'red' : 'green' }}-600"></div></code></pre>

@component('_partials.tip-good')
Do dynamically select a complete class name
@endcomponent

<pre className="language-html mt-4"><code><div className="@{{ error ? 'text-red-600' : 'text-green-600' }}"></div></code></pre>
...
<div :className="{ hidden: !isOpen, ... }">{/* ... */}</div>
```
